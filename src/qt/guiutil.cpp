...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Tokugawa"))
        return false;
...
{
    // Convert Tokugawa:// to Tokugawa:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Tokugawa://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Tokugawa:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Tokugawa.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Tokugawa.lnk
...
{
    return GetAutostartDir() / "Tokugawa.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Tokugawa.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Tokugawa\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Tokugawa app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Tokugawa app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Tokugawa-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Tokugawa-qt [" + tr("command-line options") + "]                     " + "\n";
