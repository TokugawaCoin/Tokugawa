...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Tokugawa.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Tokugawad.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeTokugawaamount=<n> " + _("Keep N Tokugawa anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Tokugawa is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Tokugawa is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Tokugawa version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Tokugawa server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Tokugawa") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Tokugawa to complete") << "\n";
    ...
    nAnonymizeTokugawaAmount = GetArg("-anonymizeTokugawaamount", 0);
    if(nAnonymizeTokugawaAmount > 999999) nAnonymizeTokugawaAmount = 999999;
    if(nAnonymizeTokugawaAmount < 2) nAnonymizeTokugawaAmount = 2;
    ...
    LogPrintf("Anonymize Tokugawa Amount %d\n", nAnonymizeTokugawaAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1TOK+1000 == (.1TOK+100)*10
       10TOK+10000 == (1TOK+1000)*10
    */
